...
#### 如果你正在阅读这行字
#### 我正在准备 v1.1 mod 的介绍与使用说明 视频
#### 以下内容均为 v1.1 mod 而写
#### v1.1 的 mod 尚未上传至 releases
#### 很快就会更新
#### 这是一个相当大的重构
------
...
补给点填写箱子方块坐标，床点填写床方块坐标。熔炉排必须沿一个坐标轴排列，只保存首尾两个熔炉位置。导航会寻找能触及箱子或熔炉的附近站位。睡觉使用更严格的专用站位搜索，会先让玩家眼睛与床中心的距离进入 3 格以内再进行交互。
...
| 导航 | `elytra_navigation_min_distance=32`、`navigation_stall_timeout_seconds=10`、`navigation_retry_count=2`、`flight_retry_count=2`、`portal_transition_cost=16`、`portal_transition_timeout_seconds=20` |
...
前去经验熔炉修复时会逐个取出产物。经验仍在吸收时会切换待维修目标；耐久停止增长后才访问下一个熔炉；所有目标修满后立即返回。禁用 `cross_dimension_repair` 后只要求配置经验熔炉的坐标，按当前维度解释，往返过程不会使用传送门。

Baritone 的疾跑和跑酷始终启用。

所有非挖掘目的地均禁止放置方块；前往待挖掘区域传送门时允许挖掘，但维修行程离开挖掘区域后的其他阶段同时禁止挖掘和放置。

适合时先尝试步行；目标较远或无法直接向上步行时使用鞘翅。导航 watchdog 会在报告错误前重试停滞路线。
...
构建脚本会从 `build.gradle` 中配置的路径读取魔改版 Baritone Fabric JAR，当前为：
